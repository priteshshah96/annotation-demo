...
export const fileApi = {
  async uploadFile(fileData) {
    try {
      ...
      fileData.content.forEach(abstract => {
        if (!abstract.paper_code || !abstract.abstract) {
          throw new Error('Each abstract must have a paper_code and abstract field');
        }
      });
  ...
  async getFiles(page = 1, limit = 10) {
    try {
      const response = await api.files.getAll({ page, limit });
      if (!response.files) {
  ...
  async getUserStats() {
    ...
    if (cachedStats && Date.now() - lastFetchTime < cacheDuration) {
      return cachedStats;
    }
